...
function setupRealtimeListeners() {
  ...
  window.onSnapshot(getColRef('CONTRACTS'), s => {
    window.CONTRACTS = s.docs.map(doc=>{let d=doc.data();return{id:d.contract_id||doc.id,name:d.project_name||'',customer:d.customer_name||'',value:Number(d.total_contract_value||d.value)||0,signDate:d.contract_sign_date||'',startDate:d.contract_start_date||'',endDate:d.end_date||'',note:d.note||'',status:d.status||'active',transactions:d.transactions||[]};});
